...
import {
    ...
    DeleteObjectOutput, DeleteObjectRequest, ObjectStorageClass
...
interface customerConfig {
    customer: string
    format: string // csv (w/ Headers), json (as key/value pairs), jsonMapped (Column Map), csvMapped (Column Map)
    map: object
...
export const tricklerQueueProcessorHandler: Handler = async (event: SQSEvent, context: Context) => {
    ...
    if (tcc.SelectiveDebug.indexOf("9,") > -1) console.log(`Selective Debug 9 - Process Environment Vars: ${JSON.stringify(process.env)}`)
    ...
    {
        console.log(`Purge Requested, Only action will be to Purge ${tcc.QueueBucketPurge} of ${tcc.QueueBucketPurgeCount} Records. `)
    ...
    if (tcc.SelectiveDebug.indexOf("11, ") > -1) console.log(`Selective Debug 11 - SQS Events Batch\n${JSON.stringify(event)}`)
    ...
    for (const q of event.Records)
    {
        ...
        if (tcc.SelectiveDebug.indexOf("11, ") > -1) console.log(`Selective Debug 11 - SQS Events BatchFail \n${JSON.stringify(sqsBatchFail)}`)
...
export const s3JsonLoggerHandler: Handler = async (event: S3Event, context: Context) => {
    ...
    {
        console.log(`Purge Requested, Only action will be to Purge ${tcc.CacheBucketPurge} of ${tcc.CacheBucketPurgeCount} Records. `)
    ...
    for (const r of event.Records)
    {
        ...
        try
        {
            ...
            if (tcc.SelectiveDebug.indexOf("3, ") > -1) console.log(`Return from Process S3 Object Content Stream - ${processS3ObjectStream}`)
...
async function getCustomerConfig (filekey: string) {
    ...
    //         )
    //             .catch((err) => {
    //                 console.log(`Exception Processing Customer Config - ${err}`)
    ...
    //                 catch (e)
    //                 {
    //                     console.log(`exception in then for Cust Config: ${e}`)
    //                 }
        ...
        .then(async (getS3StreamResult: GetObjectCommandOutput): Promise<string> => {
            ...
            s3ContentReadableStream
                ...
                .on('end', async function (msg: string) {
                    ...
                    if (tcc.SelectiveDebug.indexOf("2, ") > -1) console.log(`Selective Debug 2: End of Queueing Work for (${key}) Result: ${sqwResult}`)
...
async function storeAndQueueWork (chunks: string[], s3Key: string, config: customerConfig, batch: number) {
    ...
    if (batch > tcc.MaxBatchesWarning) console.log(`Warning: Updates from the S3 Object (${s3Key}) are exceeding (${batch}) the Warning Limit of ${tcc.MaxBatchesWarning} Batches per Object.`)
...
async function addWorkToS3ProcessStore (queueContent: string, key: string) {
    ...
    {
        console.log(`Work/Process Bucket Quiesce is in effect, no New Work Files will be written to the S3 Queue Bucket.`)
        return
    }
...
export async function postToCampaign (xmlCalls: string, config: customerConfig, count: string) {
    ...
    if (tcc.SelectiveDebug.indexOf("5,") > -1) console.log(`Selective Debug 5 - POST Updates are: ${xmlCalls}`)
...
async function getAnS3ObjectforTesting (bucket: string) {
    ...
    await s3.send(new ListObjectsV2Command(listReq))
        ...
        .catch((e) => {
            console.log(`Exception on S3 List Command for Testing Objects from ${bucket}: ${e}`)
        })
    // .finally(() => {
    //     console.log(`S3 List Finally...`)
    // })
    // } catch (e)
    // {
